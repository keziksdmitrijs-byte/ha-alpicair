...
class AlpicAirConfigFlow(config_entries.ConfigFlow,domain=DOMAIN):
    ...
    async def async_step_user(self,user_input=None)->FlowResult:
        ...
        if user_input is not None:
            await self.async_set_unique_id(f"{user_input[CONF_HOST]}:{user_input[CONF_PORT]}:{user_input[CONF_SLAVE]}"); self._abort_if_unique_id_configured(); return self.async_create_entry(title=user_input[CONF_NAME],data=user_input)
